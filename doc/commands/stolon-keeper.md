...
Note: To avoid repeating the arguments on every command (or inside scripts), all options can be exported as environment variables. Their name will be the same as the option name converted in uppercase, with _ replacing - and prefixed with STKEEPER_.

For example:
```
STKEEPER_KUBE_RESOURCE_KIND
STKEEPER_PG_PORT
STKEEPER_PG_REPL_PASSWORD
STKEEPER_PG_REPL_USERNAME
STKEEPER_PG_SU_PASSWORD
