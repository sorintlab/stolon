	...
	if p.dbLocalState.UID != db.UID {
		...
		case cluster.DBInitModePITR:
			...
			if recoveryMode == pg.RecoveryModeRecovery {
				...
				pgm.SetRecoveryOptions(p.createRecoveryOptions(pg.RecoveryModeNone, standbySettings, db.Spec.PITRConfig.ArchiveRecoverySettings, db.Spec.PITRConfig.RecoveryTargetSettings))
			}
